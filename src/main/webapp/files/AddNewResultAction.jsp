...
String sql = "INSERT INTO marks(roll,id,m1,m2,m3,m4,m5) values(?,?,?,?,?,?,?)";
	PreparedStatement ps = con.prepareStatement(sql,PreparedStatement.RETURN_GENERATED_KEYS);
	
	ps.setString(1,cmp.getRoll());
	ps.setInt(2,cmp.getId());
	ps.setFloat(3,cmp.getM1()); 
	ps.setFloat(4,cmp.getM2());
	ps.setFloat(5,cmp.getM3());
	ps.setFloat(6,cmp.getM4());
	ps.setFloat(7,cmp.getM5());
	...
	if(n>0 ){
		float percent=(cmp.getM1()+cmp.getM2()+cmp.getM3()+cmp.getM4()+cmp.getM5())/5;
		   ...
		   {   grade="A+";status="pass";}
					 sql="insert into result(mid,roll,status,percent,grade) values(?,?,?,?,?)";
						ps=con.prepareStatement(sql);
						
							ps.setInt(1,pk);
							ps.setString(2,cmp.getRoll());
							ps.setString(3,status);
							ps.setFloat(4,percent);
							ps.setString(5,grade);
							 nnn=ps.executeUpdate();
							 if(nnn>0){
								 System.out.println("done"+status);
								 jobj.addProperty("status","success");
									jobj.addProperty("message","Details of "+cmp.getRoll()+" are saved.");
							 //}
				}
		
	}else{
		jobj.addProperty("status","failed");
		jobj.addProperty("message","Details of "+cmp.getRoll()+" are not saved.");
	}
	}
catch(Exception ex){
