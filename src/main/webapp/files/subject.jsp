...
try {
	...
	String sql = "select s1,s2,s3,s4,s5,id from subjects where course='"+cmp.getCname()+"' and branch='"+cmp.getBname()+"' and sem="+cmp.getSemester()+"";
	...
	jobj.addProperty("s1", rs.getString(1));
	jobj.addProperty("s2", rs.getString(2));
	jobj.addProperty("s3", rs.getString(3));
	jobj.addProperty("s4", rs.getString(4));
	jobj.addProperty("s5", rs.getString(5));
	jobj.addProperty("id",rs.getInt(6));
